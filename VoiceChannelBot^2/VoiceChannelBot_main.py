...
async def on_ready():
    print('Ready')
    await Globals.bot.change_presence(status=discord.Status.idle, activity=discord.Activity(type=discord.ActivityType.listening, name="?help"))


@commands.Cog.listener()
async def on_voice_state_update(member: Member, _, after: VoiceChannel):
    if after == None or not Globals.started:
        return
    if after.channel == member.guild.get_channel(Globals.guilds[str(member.guild.id)]["monitorchannel"]):
        ...
        ch = await member.guild.get_channel(Globals.guilds[str(member.guild.id)]["category"]).create_voice_channel(name=Globals.guilds[str(member.guild.id)]["name"] + " " + str(count), user_limit=Globals.guilds[str(member.guild.id)]["defaultmaxmembers"])
        await member.move_to(ch)
        
        Globals.channels.append(Globals.Channel(ch, time() + Globals.guilds[str(member.guild.id)]["timeout"], member.guild, count))
