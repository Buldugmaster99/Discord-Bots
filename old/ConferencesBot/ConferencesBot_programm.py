...
class Conference(object):
    ...
    def respond(self):
        return "subject:" + self.subject.capitalize() + "  date:" + self.day.date().strftime("%d.%m.%Y") + "  start:" + times[self.time][
            0] + "  end:" + times[self.time][1] + "  maybe:" + str(self.maybe)
    ...
    def notifiy(self):
        ...
        if self.maybe:
            return (
                f"Maybe {self.subject.capitalize()} conference in {notificationtime} minutes for {self.mention}  {times[self.time][0]} - {times[self.time][1]} \nlink for conference {self.link}")
        else:
            return (
                f"{self.subject.capitalize()} conference in {notificationtime} minutes for {self.mention}  {times[self.time][0]} - {times[self.time][1]} \nlink for conference: {self.link}")
...
async def notifiy():
    ...
    if (not dailyall and t.weekday() < 5) and (
            (t.hour == int(dailyalltimes[0]) and t.minute >= int(dailyalltimes[1])) or (13 > t.hour > int(dailyalltimes[0]))):
        dailyall = True
        prmap = []
    ...
    for Conference in Conferences:
        if Conference.day.strftime("%d.%m.%Y") == t.strftime("%d.%m.%Y"):
            if (t + datetime.timedelta(minutes=notificationtime)).strftime("%H.%M") == times[Conference.time][
                0] and not Conference.notified:
                mess = await notifychannel.send(Conference.notifiy())
                await mess.publish()
                print("\n" + Conference.notifiy())
        if Conference.day.strftime("%d.%m.%Y") == t.strftime("%d.%m.%Y") and t.strftime("%H.%M") == (
                times[Conference.time][1].split('.')[0].zfill(2) + "." + str(int(times[Conference.time][1].split('.')[1]) + 5).zfill(2)):
